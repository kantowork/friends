...
final class ReadReceiptServiceTests: XCTestCase {
    ...
    func testDirectMessageReadStatusCalculation() {
        ...
        // Populate messageService readReceipts map
        messageService.readReceipts[chatId] = [
            friendUid: bobReceipt
        ]
        
        // 1. Message sent at t0 (before Bob's lastReadAt) -> Should be READ
        let isReadT0 = messageService.isMessageRead(chatId: chatId, messageDate: t0, senderId: myUid)
        ...
        let isReadT1 = messageService.isMessageRead(chatId: chatId, messageDate: t1, senderId: myUid)
        ...
        let isReadT2 = messageService.isMessageRead(chatId: chatId, messageDate: t2, senderId: myUid)
    ...
    func testGroupChatReadCountCalculation() {
        ...
        let countT0 = messageService.readCountForMessage(chatId: chatId, messageDate: t0, senderId: myUid)
        XCTAssertEqual(countT0, 2, "t0時点のメッセージはBobとCarolの2名が既読である必要があります")
        
        // Message sent at t1 -> Bob and Carol have read it (count = 2)
        let countT1 = messageService.readCountForMessage(chatId: chatId, messageDate: t1, senderId: myUid)
        XCTAssertEqual(countT1, 2, "t1時点のメッセージはBobとCarolの2名が既読である必要があります")
        
        // Message sent at t2 -> Only Bob has read it (count = 1)
        let countT2 = messageService.readCountForMessage(chatId: chatId, messageDate: t2, senderId: myUid)
        XCTAssertEqual(countT2, 1, "t2時点のメッセージはBobの1名のみ既読である必要があります")
        
        // Message sent at t2 + 100 -> Nobody has read it (count = 0)
        let countT3 = messageService.readCountForMessage(chatId: chatId, messageDate: Date(timeIntervalSince1970: 3100), senderId: myUid)
