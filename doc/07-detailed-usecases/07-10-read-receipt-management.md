...
3. **ホーム画面およびチャット一覧での未読判定 (Fan-out on Read)**:
   - **大規模グループ対応原則**: 1万人規模のグループでも書き込みが破綻しないよう、メッセージ送信時に受信者全員へ未読フラグを配るファンアウト書き込み（Fan-out on Write）は**一切行わない**。
   - **送信時の責務**: メッセージ送信時、Workers はチャット本体（`/chats/{chatId}`）の `lastMessageAt` および `lastMessageId` を 1 回更新する（書き込みコスト完全一定・2 Writes）。
   - **未読判定アルゴリズム**:
     - ユーザーの参加チャット一覧（通常 5〜30 件）をリアルタイム購読。
     - 各チャット $C$ に対し、自身の既読時刻 $T_{\text{read}} = \text{myReceipts}[C.\text{id}].\text{lastReadAt}$ をローカルで比較。
     - $C.\text{lastMessageAt} > T_{\text{read}}$ を満たすチャットを **「未読チャット」** として抽出。
   - **ホーム画面（A01）表示**:
     - 未読チャットが存在する場合、ホーム画面の「未読メッセージ / 未読チャット」セクションにカード一覧を表示。
     - チャットを開いて $T_{\text{read}}$ が更新された瞬間に、未読一覧からリアルタイムに除外される。
