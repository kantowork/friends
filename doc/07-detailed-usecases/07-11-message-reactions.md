...
### 1.1 提供リアクション種別（8種類）
ユーザーが直感的に感情・意思を表明できるよう、以下のリアクションを提供します。メッセージ長押し時のクイックアクションには、利用頻度の高い 7 種類（👍, ❤️, 🆗, 😊, 🤣, 😢, 😱）を直接表示します。
※ 「…」ボタンによるその他の絵文字（🤔 等）や一覧の展開は、今後の実装フェーズにて提供予定です。
...
| `laugh` | 🤣 | 大笑い | Laughing | 面白い、大笑い、爆笑 |
| `sad` | 😢 | かなしい | Sad | 悲しい、残念 |
| `surprised` | 😱 | びっくり | Surprised | 驚き、意外、ショック |
| `thinking` | 🤔 | 考え中 | Thinking | 検討中、疑問、熟考（今後の絵文字一覧用） |
...
2. **リアクション詳細画面（オンデマンド購読・モーダル表示中のみ）**:
   - ユーザーがメッセージのリアクションバッジをタップして詳細シートを開いた際にのみ、該当メッセージの `/reactions` サブコレクションをリアルタイム購読（`watchReactionsByMessageId`）し、モーダルを閉じた瞬間に即時リスナーを破棄（`remove()`）。
   - さらに、Firestoreへの書き込み完了前であっても、ローカルで保持する自身の最新リアクション状態（`userReactions`）を即座にリストへ反映し、「リアクションしたのに空になる」表示ズレを完全に防止。
...
3. **長押しクイックアクションメニュー（リアクション＆コピー）**:
   - メッセージを長押しすると、メッセージの直下に一体型のアクションカードがアニメーション表示。
   - システム標準の `.contextMenu` は使用せず、軽量・直感的な独自UIで提供。
   - **上段（絵文字リアクション）**: 7種類の絵文字（👍, ❤️, 🆗, 😊, 🤣, 😢, 😱）が水平に配置され、タップで即座にリアクションをトグル（付与/解除）しメニューが閉じる。OSの文字サイズ設定（Dynamic Type）に応じて絵文字サイズが拡大縮小し、かつHIG推奨の最小タッチターゲット領域（44×44pt）を担保する。
     - （※「…」ボタンによるその他の絵文字一覧展開は今後の実装予定とし、現段階ではUI非表示）
   - **下段（コピー）**: メッセージ本文をクリップボードにコピーし、成功ハプティクスを発火してメニューが閉じる。OSの文字サイズ設定（Dynamic Type）に応じてフォント・アイコンが追従し、最小タッチ領域（高さ44pt以上）を担保する。※写真添付メッセージ（本文テキストがない場合等）では、コピー行および境界線（Divider）は非表示とし絵文字リアクションのみを表示する。
4. **リアクションバッジ**:
   - リアクションが付与されたメッセージの吹き出し下部に、付与されているリアクション絵文字と数量をカプセル型バッジで一覧表示。
   - **数量表示ルール**: リアクション数が 1 の場合は件数数字（`1`）を表示せず絵文字アイコンのみを表示し、2 以上のときのみ数量（例: `👍 2`、`❤️ 3`）を表示する。
   - **文字サイズ比例スケーリング**: 設定で選択されたメッセージ文字サイズに連動してバッジの絵文字・数量・パディング・角丸が自然に比例拡大縮小する。
   - 自分がリアクションしているものはハイライト表示（青枠/青背景）。
5. **リアクション詳細モーダル (Sheet)**:
   - メッセージのリアクションバッジタップからシートを表示。
