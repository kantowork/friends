...
1. **`ToastNotificationManager` (`@ObservableObject`)**:
   ...
   - **ナビゲーション遷移方式（一覧へ戻れるプッシュ遷移）**:
     - トースト通知タップ時、モーダル表示（`.sheet`）ではなく、該当チャットの種別（1:1 DM または かいぎ）に応じて `MainTabView` の該当タブ（`ともだち` または `かいぎ`）へ切り替え、該当タブの `NavigationStack(path:)` にチャットモデルをプッシュ遷移する。
     - これにより、チャット詳細画面左上の戻るボタン（`< ともだち` または `< かいぎ`）をタップすることで、一覧画面へ自然に戻ることができる。
2. **`ToastBannerView`**:
