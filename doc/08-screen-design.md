...
- **セクション構成**:
  ...
  4. **アプリ情報**:
     - `アプリ情報`: タップで `B06` アプリ情報モーダル画面を表示（バージョン、暗号化方式、ライセンス表示）
...
- **セクション構成**:
  ...
  3. **アプリ情報セクション**:
     - **アプリ情報**: 行タップで `B06` アプリ情報モーダル画面を表示 (`アプリ情報` + 右端 `chevron.right`)
- **確認ダイアログ & アラート**:
...
### B06 アプリ情報モーダル画面 (`AppInfoView`)

- **遷移方式**: `B02` 設定画面または `B03` プロフィール編集画面からシート（モーダル）表示
- **ナビゲーションバー**:
  - タイトル: `アプリ情報` (`L10n.AppInfo.title`)
  - 右（または左）: `閉じる` ボタン（`L10n.Common.close`）
- **セクション構成**:
  1. **バージョン情報**:
  ...
  3. **オープンソースライセンス**:
     - 行: `ライセンス表示` (`L10n.AppInfo.licenses`) + 右端 `chevron.right`
     - タップで `B06-a` オープンソースライセンス一覧画面へ Navigation 遷移

---

### B06-a オープンソースライセンス画面 (`LicenseListView` / `LicenseDetailView`)

- **ライセンス一覧 (`LicenseListView`)**:
  - タイトル: `オープンソースライセンス` (`L10n.AppInfo.licenseListTitle`)
  - 検索バー (`.searchable`): パッケージ名で絞り込み可能
  - リスト表示: SPM 依存関係から自動生成された OSS パッケージをアルファベット順に表示（各行にライブラリ名、バージョン、chevron）
- **ライセンス詳細 (`LicenseDetailView`)**:
  - タイトル: ライブラリ名
  - 上部メタデータ: バージョン、リポジトリへのリンク (`Link`)
  - ライセンス全文: 等幅フォントでスクロール表示されるライセンス条文（MIT, Apache 2.0, BSD 等）
...
- **タブ切替**: `二次元コード` / `テキスト`
- **二次元コードタブ**:
  - 上部: カメラプレビュー（相手の二次元コードスキャン。正しいフォーマットの二次元コードをスキャンすると自動で友達追加を実行）
