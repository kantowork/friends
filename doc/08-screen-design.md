...
| A03m | テナント選択         | テナントの指定 (2次元コードスキャン/JSON入力/URL指定)  |
...
| C03  | ともだち追加         | 統合1画面（上: カメラスキャン / 下: 自2次元コード+3桁合言葉）& テキスト連携 |
...
flowchart TD
  ...
  B04["B04 パスワード変更"]
  B05["B05 ふっかつのじゅもん"]
  A03m["A03m テナント追加 (2次元コード/URL/テキスト)"]

  C01["C01 ともだち一覧 (タブ: ともだち)"]
  C02["C02 チャット詳細 (1:1 / かいぎ E2EE)"]
  C03["C03 ともだち追加 (2次元コード/合言葉/テキスト)"]
...
- **コンテンツ構成**:
  1. **入力方式切替**: セグメンテッドピッカー（「2次元コード」「URL」「直接入力」）
  2. **タブコンテンツ**:
     - 2次元コード: カメラプレビュー（実機）/ シミュレーター代替表示
...
- **コンテンツ構成**:
  ...
  2. **テナントの追加ボタン**:
     ...
     - タップで既存の `A03m` テナント選択画面（2次元コード / URL / テキスト入力）をモーダル表示。
...
- **コンテンツ構成**:
  - `shared/legal/licenses.md` を `LegalMarkdownView(fileName: "licenses")` でレンダリング。
  ...
  2. **新着メッセージフローティングバナー (オンデマンド移動)**:
   ...
   3. **入力バー (`[写真] [入力欄] [送信]`)**:
      - 左側: 写真添付ボタン（写真アイコン `photo.on.rectangle.angled`）
        ...
        - 「写真を撮る」: カメラ起動（撮影完了後に速やかに自動送信）
        - 「写真を選択」: OS標準の写真ピッカー（`PhotosPicker`、複数選択対応・上限10枚。選択完了後に速やかに自動送信）
...
- **タブ切替**: `2次元コード` / `テキスト`
- **2次元コードタブ**:
  - 上部: カメラプレビュー（相手の2次元コードスキャン。正しいフォーマットの2次元コードをスキャンすると自動でともだち追加を実行）
  - 下部: 自分の招待2次元コード（余計な文字・ボタンなしのミニマル30秒更新表示）
- **テキストタブ**:
  - **自分の情報エリア**:
