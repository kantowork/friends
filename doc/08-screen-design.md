...
| A06m | 通知許可案内         | 初回ログイン後のメッセージ通知事前説明シート（モーダル） |
...
- **コンテンツ構成**:
  ...
  2. **未読メッセージセクション**:
     - **未読チャット一覧カード**: 未読が存在するチャット（`C.lastMessageAt > myReceipt.lastReadAt`）を最新順に一覧表示。各行にアイコン、相手名/グループ名、最新メッセージ時刻、および未読バッジを表示。タップで当該チャット（`C02`）へダイレクト遷移。
     - **未読なし時の表示**: 未読チャットが存在しない場合、未読セクション自体を非表示（またはクリーンな空状態表示）とし、画面をすっきり保つ。
  3. **お知らせセクション**:
     - お知らせ一覧 / システムお知らせ（未読なし時はプレースホルダー表示、ベルアイコン `.font(.system(.largeTitle))`、テキスト `.font(.subheadline)`）。
...
- **セクション構成**:
  ...
  3. **アプリ設定**:
     - `通知`: トグルスイッチ（ON / OFF）。プッシュ通知の受信・配信設定を切り替え。
...
### A06m 通知許可案内シート (`NotificationPermissionSheet`)

- **遷移方式**: 初回ログイン完了時、未登録の場合にアプリ独自デザインのハーフモーダル（`.sheet`）として表示
- **コンテンツ構成**:
  - **ベルアイコン / イラスト**: ブランドカラーのアニメーションアイコン
  - **タイトル**: `通知をオンにしませんか？` (`L10n.Notification.permissionTitle`)
  - **説明文**: `新着メッセージの受信にすばやく気づけるようになります。` (`L10n.Notification.permissionDesc`)
  - **アクションボタン**:
    - **「通知をオンにする」ボタン** (`L10n.Notification.enableButton`): プライマリスタイル。タップ時に iOS 公式許諾ダイアログを起動し、トークンを Firestore に登録。
    - **「あとで」ボタン** (`L10n.Notification.laterButton`): セカンダリスタイル。システムダイアログを出さずにシートを閉じる。
