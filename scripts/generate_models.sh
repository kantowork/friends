...
# 2. shared/schema/*.ts (Zod REST API SSoT) -> scripts/generate_api.mjs による Swift Codable 自動生成
# を一括同期実行します。
...
echo "🔄 [Friends Model Generator] 2. shared/schema/*.ts (Zod SSoT) から Swift API モデルを自動生成しています..."
